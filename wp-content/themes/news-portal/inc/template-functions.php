...
function news_portal_scripts() {
    ...
    wp_enqueue_style( 'news-portal-responsive-style', get_template_directory_uri().'/assets/css/np-responsive.css?v=1.3', array(), esc_attr( $news_portal_version ) );
            ...
            foreach ( $get_decode_social_media as $single_icon ) {
                ...
                if ( !empty( $icon_url ) ) {
                  if (is_numeric($icon_url)){
                    echo '<span class="social-link"><a href="'. 'https://wa.me/'.$icon_url .'" target="_blank"><i class="'. esc_attr( $icon_class ) .'"></i></a></span>';
                  }else{
